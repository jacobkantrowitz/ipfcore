...
varOfInt <- "Macrophage.CD68."

dds2 <- dds
m2 <- model.matrix(~TISSUE_DIAGNOSIS + TISSUE_DIAGNOSIS:NESTED_PATIENT + Neutrophil + Bcell.CD79a. + CD4 + CD8 + Eosinophils, colData(dds2))
m2 <- m2[ ,-which(apply(unname(m2), 2, sum)==0)]

m3 <- model.matrix(~TISSUE_DIAGNOSIS + TISSUE_DIAGNOSIS:NESTED_PATIENT + Neutrophil + Bcell.CD79a. + CD4 + CD8 + Eosinophils + Macrophage.CD68., colData(dds2))
m3 <- m3[ ,-which(apply(unname(m3), 2, sum)==0)]


deseqAnalysis(dds2, mod.full=m3, mod.reduced=m2, varOfInt = varOfInt, fdr = 0.25,
              filename = "BU_All_FullModel_MacrophageCD68",
